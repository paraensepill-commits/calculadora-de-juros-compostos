...
export const ResultsTable: React.FC<ResultsTableProps> = ({ data }) => {
  return (
    <div className="mt-8">
      <h3 className="text-center font-bold text-xl text-[#00bd56] mb-4">Tabela:</h3>
      <div className="bg-white rounded-lg shadow-sm border border-[#00bd56]/20 overflow-hidden">
        <div className="overflow-x-auto custom-scrollbar max-h-[500px]">
          <table className="w-full text-sm text-left">
            <thead className="bg-[#00bd56] text-white font-semibold sticky top-0 z-10">
              <tr>
            ...
            <tbody className="divide-y divide-gray-100">
              {data.map((row) => (
                <tr key={row.month} className="hover:bg-[#00bd56]/5 transition-colors duration-150">
                  ...
                  <td className="px-6 py-4 text-right font-bold text-[#00bd56]">{formatCurrency(row.totalAccumulated)}</td>
                </tr>
              ))}
            </tbody>
          </table>
